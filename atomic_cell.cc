...
static imr::utils::object<data::cell::structure> copy_cell(const data::type_imr_state& imr_data, const uint8_t* ptr)
{
...
bool atomic_cell_or_collection::equals(const abstract_type& this_type, const atomic_cell_or_collection& other, const abstract_type& other_type) const
{
    using dc = data::cell;
    ...
    if (!flags_a.get<dc::tags::external_data>() && !flags_b.get<dc::tags::external_data>()) {
        auto ctx_a = dc::context(flags_a, this_type.imr_state().type_info());
        auto ctx_b = dc::context(flags_b, other_type.imr_state().type_info());
    ...
    if (this_type.is_atomic()) {
        auto a = atomic_cell_view::from_bytes(this_type.imr_state().type_info(), _data);
        auto b = atomic_cell_view::from_bytes(other_type.imr_state().type_info(), other._data);
...
size_t atomic_cell_or_collection::external_memory_usage(const abstract_type& t) const
{
    ...
    // FIXME: This is much more expensive than it used to be, which may be one
    // of the reasons why read performance has taken a hit. Perhaps we should
    // keep the size in memory after all.
