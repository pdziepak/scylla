...
private:
    ...
    void expand(size_t new_capacity) {
        auto ptr = static_cast<T*>(std::aligned_alloc(alignof(T), new_capacity * sizeof(T)));
        if (!ptr) {
            throw std::bad_alloc();
        }
    ...
    iterator insert(const_iterator cpos, InputIterator first, InputIterator last) {
        if constexpr(std::is_base_of_v<std::forward_iterator_tag, typename std::iterator_traits<InputIterator>::iterator_category>) {
            ...
            if (__builtin_expect(pos == end(), true)) {
                ...
                std::uninitialized_move(pos, end(), end() + new_count - after);
                ...
                try {
                    ...
                    try {
                        ...
                        std::destroy(end(), end() + new_count - after);
                        throw;
                    }
                } catch (...) {
                    std::move(end() + new_count - after, end() + new_count, pos);
                    std::destroy(end() + new_count - after, end() + new_count);
                    throw;
                }
