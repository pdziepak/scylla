...
void memtable::encoding_stats_collector::update(tombstone tomb) {
    if (tomb) {
...
void memtable::encoding_stats_collector::update(const range_tombstone& rt) {
    update(rt.tomb);
}

void memtable::encoding_stats_collector::update(const row_marker& marker) {
