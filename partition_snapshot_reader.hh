...
class partition_snapshot_flat_reader : public flat_mutation_reader::impl, public MemoryAccounter {
    ...
    // snapshot, refernces to region and alloc section) or dropped on any
    ...
    public:
        ...
        // to be engaged and equal the postition of the row returned last time.
        // If the ck_range is different or this is the first call to this
        // function last_row has to be disengaged. Additionaly, when entering
...
private:
    ...
    mutation_fragment_opt read_next() {
        auto nrow = (!_next_row && !_no_more_rows_in_current_range)
                  ? _reader.next_row(*_current_ck_range, _last_entry, _range_tombstones)
                  : std::exchange(_next_row, {});
        if (nrow) {
            auto mf = _range_tombstones.get_next(nrow->as_clustering_row().position());
            if (mf) {
                _next_row = std::move(nrow);
                return mf;
            }
            _last_entry = position_in_partition(nrow->as_clustering_row().position());
            return nrow;
        } else {
