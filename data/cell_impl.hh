...
public:
    ...
    auto write_all_to_destination() {
        return [this] (uint8_t* out) noexcept {
            while (_it != _end) {
                _current = *_it++;
                out = std::copy_n(_current.data(), _current.size(), out);
            }
        };
    }
    ...
    auto write_to_destination(size_t n) {
        return [this, n] (uint8_t* out) mutable noexcept {
            while (n) {
                ...
                out = std::copy_n(_current.data(), this_size, out);
...
{
    return do_write_value<value_writer<std::decay_t<FragmentRange>>>(value.size(), value, force_internal);
}
...
inline basic_value_view<is_const> cell::variable_value::do_make_view(structure::basic_view<is_const> view, bool external_storage)
{
    ...
    return view.template get<tags::value_data>().visit(utils::make_visitor(
            [&] (imr::pod<uint8_t*>::view ptr) {
                ...
                if (size > maximum_external_chunk_length) {
                    ...
                    return basic_value_view<is_const>(ex_view.get<tags::chunk_data>(ex_ctx), size - maximum_external_chunk_length, next);
                } else {
                    ...
                    return basic_value_view<is_const>(ex_view.get<tags::chunk_data>(ex_ctx), 0, nullptr);
                }
            },
            [] (imr::buffer<tags::data>::basic_view<is_const> data) {
                return basic_value_view<is_const>(data, 0, nullptr);
            }
    ), ctx);
}
