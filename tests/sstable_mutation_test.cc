...
SEASTAR_THREAD_TEST_CASE(test_reading_serialization_header_and_encoding_stats) {
    auto dir = make_lw_shared<tmpdir>();
    storage_service_for_tests ssft;
    auto wait_bg = seastar::defer([] { sstables::await_background_jobs().get(); });
    ...
    auto stats = sst->get_encoding_stats_for_compaction(*s);
    BOOST_CHECK_EQUAL(stats.static_columns.size(), 1);
    BOOST_CHECK(stats.static_columns[0]);
    BOOST_CHECK_EQUAL(stats.regular_columns.size(), 2);
    BOOST_CHECK(stats.regular_columns[0]);
    BOOST_CHECK(!stats.regular_columns[1]);
    ...
    stats = sst->get_encoding_stats_for_compaction(*s2);
    BOOST_CHECK_EQUAL(stats.static_columns.size(), 1);
    BOOST_CHECK(!stats.static_columns[0]);
    BOOST_CHECK_EQUAL(stats.regular_columns.size(), 3);
    BOOST_CHECK(!stats.regular_columns[0]);
    BOOST_CHECK(stats.regular_columns[1]);
    BOOST_CHECK(!stats.regular_columns[2]);
}
