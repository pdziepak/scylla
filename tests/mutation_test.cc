...
static mutation_partition get_partition(memtable& mt, const partition_key& key) {
    static thread_local std::deque<dht::partition_range> ranges_storage;
    auto dk = dht::global_partitioner().decorate_key(*mt.schema(), key);
    auto reader = mt.make_flat_reader(mt.schema(), ranges_storage.emplace_back(dht::partition_range::make_singular(dk)));
