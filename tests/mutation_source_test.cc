...
public:
    ...
    using value = std::variant<bytes, std::vector<std::tuple<bytes, bytes>>>;
    using cell = std::tuple<sstring, value>;
    using row = std::vector<cell>;
    using clustered_row = std::tuple<api::timestamp_type, row>;

private:
    key _partition_key;
    row _static_row;
    std::map<key, clustered_row> _clustered_rows;
    std::vector<std::pair<key, key>> _range_tombstones;
...
public:
    ...
    void add_static_cell(const sstring& column, value v) {
        _static_row.emplace_back(column, std::move(v));
    }

    void add_clustered_cell(const key& ck, const sstring& column, value v) {
        std::get<row>(_clustered_rows[ck]).emplace_back(column, std::move(v));
    }

    void add_clustered_row_marker(const key& ck) {
        std::get<api::timestamp_type>(_clustered_rows[ck]) = data_timestamp;
    }
    ...
    void remove_regular_column(const sstring& name) {
        for (auto& [ ckey, cr ] : _clustered_rows) {
            remove_column(std::get<row>(cr), name);
        }
    }

    void add_range_tombstone(const key& start, const key& end) {
        _range_tombstones.emplace_back(start, end);
    }
        ...
        for (auto& [ column, value_or_collection ] : _static_row) {
            ...
            std::visit(make_visitor(
                [&] (bytes value) {
                    ...
                    m.set_static_cell(*cdef, atomic_cell::make_live(*cdef->type, data_timestamp, std::move(value)));
                },
                [&] (std::vector<std::tuple<bytes, bytes>> collection) {
                    assert(!cdef->is_atomic());
        ...
        for (auto& [ ckey, cr ] : _clustered_rows) {
            ...
            for (auto& [ column, value_or_collection ] : cells) {
                ...
                std::visit(make_visitor(
                    [&] (bytes value) {
                        assert(cdef->is_atomic());
                        m.set_clustered_cell(ck, *cdef, atomic_cell::make_live(*cdef->type, data_timestamp, std::move(value)));
                    },
                    [&] (std::vector<std::tuple<bytes, bytes>> collection) {
                        assert(!cdef->is_atomic());
                        auto ctype = static_pointer_cast<const collection_type_impl>(cdef->type);
...
public:
    ...
    using removed_column = std::tuple<sstring, data_type, api::timestamp_type>;
...
private:
    ...
    void add_old_column(const sstring& name, data_type type) {
        _removed_columns.emplace_back(name, type, previously_removed_column_timestamp);
    }

    void remove_column(std::vector<column>& columns, const sstring& name) {
        auto it = find_column(columns, name);
        assert(it != columns.end());
        _removed_columns.emplace_back(name, std::get<data_type>(*it), column_removal_timestamp);
        columns.erase(it);
    }
...
public:
    ...
    void add_static_column(const sstring& name, data_type type) {
            BOOST_TEST_MESSAGE(format("Adding static column \'{}\' of type \'{}\'.", name, type->as_cql3_type()->to_string()));
        add_column(_static_columns, name, type);
    }

    void add_regular_column(const sstring& name, data_type type) {
        BOOST_TEST_MESSAGE(format("Adding regular column \'{}\' of type \'{}\'.", name, type->as_cql3_type()->to_string()));
    ...
    void remove_static_column(const sstring& name) {
        BOOST_TEST_MESSAGE(format("Removing static column \'{}\'.", name));
    ...
    void remove_regular_column(const sstring& name) {
        BOOST_TEST_MESSAGE(format("Removing regular column \'{}\'.", name));
    ...
    void rename_partition_column(const sstring& from, const sstring& to) {
        BOOST_TEST_MESSAGE(format("Renaming partition column \'{}\' to \'{}\'.", from, to));
    ...
    std::tuple<schema_ptr, std::vector<mutation>> build() const {
        auto s = build_schema();
        return std::tuple(s, build_mutations(s));
    }
};
...
void for_each_schema_change(std::function<void(schema_ptr, const std::vector<mutation>&,
    ...
    int32_t key_id = 0;
    auto random_key = [&] () -> tests::data_model::mutation_description::key {
        return { random_int32_value(), random_int32_value(), int32_type->decompose(key_id++), };
    };
    auto random_map = [&] () -> std::vector<std::tuple<bytes, bytes>> {
        return {
    ...
    auto s = tests::data_model::table_description({ { "pk1", int32_type }, { "pk2", int32_type }, { "pk3", int32_type }, },
                                                  { { "ck1", int32_type }, { "ck2", int32_type }, { "ck3", int32_type }, });
    ...
    for (auto& [id, type, alter_to, data_generators, old_type] : static_columns) {
        ...
        for (auto& dg : data_generators) {
            auto m = tests::data_model::mutation_description(random_key());
    ...
    for (auto& [id, type, alter_to, data_generators, old_type] : regular_columns) {
        ...
        for (auto& dg : data_generators) {
            m.add_clustered_cell(random_key(), name, dg());
        }
    }
    s.unordered_mutations().emplace_back(std::move(m));
    ...
    for (auto i = 0u; i < max_generator_count; i++) {
        ...
        for (auto& [id, type, alter_to, data_generators, old_type] : regular_columns) {
            auto name = format("r{}", id);
            m.add_clustered_cell(random_key(), name, data_generators[std::min<size_t>(i, data_generators.size() - 1)]());
        }

        m.add_range_tombstone(random_key(), random_key());
        m.add_range_tombstone(random_key(), random_key());
        m.add_range_tombstone(random_key(), random_key());
    ...
    std::vector<std::tuple<schema_ptr, std::vector<mutation>>> schemas;
    schemas.emplace_back(base);

    auto test_mutated_schemas = [&] {
        for (auto&& mutated : schemas) {
            fn(std::get<schema_ptr>(base), std::get<std::vector<mutation>>(base),
            std::get<schema_ptr>(mutated), std::get<std::vector<mutation>>(mutated));
        }
        for (auto i = 2u; i < schemas.size(); i++) {
            auto& base = schemas[i - 1];
            auto& mutated = schemas[i];
            fn(std::get<schema_ptr>(base), std::get<std::vector<mutation>>(base),
            std::get<schema_ptr>(mutated), std::get<std::vector<mutation>>(mutated));
        }
        schemas.clear();
    ...
    BOOST_TEST_MESSAGE("Reset to no regular columns");
    ...
    for (auto i = 1; i <= 3; i++) {
        s.alter_clustering_column_type(format("ck{}", i), varint_type);
        schemas.emplace_back(s.build());
    }
    for (auto i = 1; i <= 3; i++) {
