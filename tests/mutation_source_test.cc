...
public:
    ...
    std::tuple<sstring, schema_ptr, std::vector<mutation>> build() const {
        auto s = build_schema();
        return std::tuple(boost::algorithm::join(_change_log, "\n"), s, build_mutations(s));
    }
};
...
void for_each_schema_change(std::function<void(schema_ptr, const std::vector<mutation>&,
    ...
    std::vector<std::tuple<sstring, schema_ptr, std::vector<mutation>>> schemas;
