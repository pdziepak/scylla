...
// FIXME: rename to mutation_fragment
...
public:
    clustering_row()
        ...
        , _row_4(_schema.make_row(_key, _value_4))
        , _row_4k(_schema.make_row(_key, _value_4k))
        , _row_1M(_schema.make_row(_key, _value_1M))
    { }
    ...
    mutation_fragment make_clustering_row_4() const {
        return _schema.make_row(_key, _value_4);
    }

    mutation_fragment make_clustering_row_4k() const {
        return _schema.make_row(_key, _value_4k);
    }

    mutation_fragment make_clustering_row_1M() const {
        return _schema.make_row(_key, _value_1M);
    }
};
