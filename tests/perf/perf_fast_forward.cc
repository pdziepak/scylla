...
GCC6_CONCEPT(
requires std::is_same_v<std::invoke_result_t<Function>, test_result>
    || std::is_same_v<std::invoke_result_t<Function>, std::vector<test_result>>
)
void run_test_case(Function&& fn) {
    auto do_run = [&] {
        on_test_case();
        if constexpr (std::is_same_v<std::invoke_result_t<Function>, test_result>) {
    ...
    double iteration_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() / 1'000'000'000.;
...
int main(int argc, char** argv) {
    ...
    app.add_options()
        ...
        ("data-directory", bpo::value<sstring>()->default_value("/tmp/perf_large_partition_data"), "Data directory")
        ;
