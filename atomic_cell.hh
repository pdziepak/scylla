...
public:
    using pointer_type = std::conditional_t<is_const == const_view::yes, const uint8_t*, uint8_t*>;
protected:
...
public:
    operator basic_atomic_cell_view<const_view::yes>() const noexcept {
        return basic_atomic_cell_view<const_view::yes>(_view);
    }
    ...
    data::basic_value_view<is_const> value() const {
        return _view.value();
    }
    // Can be called on live cells only
...
class atomic_cell_view final : public basic_atomic_cell_view<const_view::yes> {
    atomic_cell_view(const data::type_info& ti, const uint8_t* data)
        : basic_atomic_cell_view<const_view::yes>(ti, data) {}

    template<const_view is_const>
    atomic_cell_view(data::cell::basic_atomic_cell_view<is_const> view)
        : basic_atomic_cell_view<const_view::yes>(view) { }
    friend class atomic_cell;
public:
...
class atomic_cell_mutable_view final : public basic_atomic_cell_view<const_view::no> {
    atomic_cell_mutable_view(const data::type_info& ti, uint8_t* data)
        : basic_atomic_cell_view<const_view::no>(ti, data) {}
public:
...
class atomic_cell final : public basic_atomic_cell_view<const_view::no> {
    using imr_object_type =  imr::utils::object<data::cell::structure>;
    imr_object_type _data;
    atomic_cell(const data::type_info& ti, imr::utils::object<data::cell::structure>&& data)
        : basic_atomic_cell_view<const_view::no>(ti, data.get()), _data(std::move(data)) {}
public:
    ...
    void swap(atomic_cell& other) noexcept {
        basic_atomic_cell_view<const_view::no>::swap(other);
